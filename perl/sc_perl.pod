...
sc_perl -- A Perl Plugin for Sylpheed-Claws


=head1 DESCRIPTION

This plugin provides an extended filtering engine for the email
client Sylpheed-Claws. It allows for the use of full Perl power
in email filters.
...
in Sylpheed-Claws' config directory (usually $HOME/.sylpheed). If
that file doesn't exist on plugin start, an empty one is created.
This file, which doesn't need to start with a sha-bang
(!#/bin/perl), holds the Perl instructions for your email
filters. To encourage some good manners, the code is executed in
a C<use strict;> environment.

Both Sylpheed-Claws' filtering B<conditions> and B<actions> are
...
C<all>, C<marked>, C<unread>, C<deleted>, C<new>, C<replied>,
C<forwarded>, C<locked>, C<colorlabel>, C<match>, C<matchcase>,
C<regexp>, C<regexpcase>, C<test>, C<size_greater>,
C<size_smaller>, C<size_equal>, C<score_greater>, C<score_lower>,
C<score_equal>, C<age_greater>, C<age_lower>, C<partial>

=item Standard Filtering Actions

C<mark>, C<unmark>, C<dele>, C<mark_as_unread>, C<mark_as_read>,
C<lock>, C<unlock>, C<move>, C<copy>, C<color>, C<execute>,
C<hide>, C<set_score>, C<change_score>, C<stop>, C<forward>,
C<forward_as_attachment>, C<redirect>

=item Fun stuff

C<SA_is_spam>, C<extract_addresses>, C<move_to_trash>, C<abort>,
C<addr_in_addressbook>, C<from_in_addressbook>, C<get_attribute_value>,
...
Delete the message. Note the name change of Sylpheed-Claws'
...
the same that Sylpheed-Claws uses. You can copy & paste from the
...
Returns true if the email address EMAIL is in the addressbook
...
Perl's own 'exit' command, since it would exit Sylpheed-Claws.
...
sure you get the idea..
 ...
 learn_and_move '#mh/mail/MailLists/Sylpheed-Claws/user'
   if matchcase('sender','sylpheed-claws-users-admin@lists.sourceforge.net');
...
every time mail is received, so manual filtering with this plugin
is not yet possible. This will change once a fitting hook is
available.
