...
in Sylpheed-Claws' config directory (usually $HOME/.sylpheed --
try `sylpheed --config-dir' if you're unsure). If that file
...
VERBOSITY is omtted, the filter logfile verbosity is not changed.
...
The messages are logged in the file B<perl_filter.log>, which
resides in the same directory as the filter scriptfile
itself. The default log level is 2. Log level 3 is not
...
To prevent the logfile from growing to big, it is truncated to
zero at plugin start.

The first time you unload this plugin (or shut down
Sylpheed-Claws), a section called B<[PerlPlugin]> will be created
in Sylpheed-Claws' configuration file B<sylpheedrc>, containing
two variables:

 * filter_log_verbosity

 * truncate_filter_logfile_on_plugin_load

If you want to change the default behaviour, you can edit these
lines. Make sure Sylpheed-Claws is not running while you do
this.
