...
enum { INTERPRETER_MESSAGE, DOCUMENT_CHANGED, LAST_SIGNAL };
...
static void gtk_gs_unrealize (GtkWidget *widget);
...
gtk_gs_class_init (GtkGSClass *klass)
{
	...
	gtk_gs_signals[DOCUMENT_CHANGED] = gtk_signal_new(
                       "document_changed", GTK_RUN_LAST, object_class->type,
                       GTK_SIGNAL_OFFSET(GtkGSClass, document_changed),
        ...
        widget_class->unrealize     = gtk_gs_unrealize;
        ...
        widget_class->set_scroll_adjustments_signal =
                gtk_signal_new ("set_scroll_adjustments",
                                GTK_RUN_LAST,
                                object_class->type,
                                GTK_SIGNAL_OFFSET(GtkGSClass, set_scroll_adjustments),
...
gtk_gs_munge_adjustments (GtkGS *gs)
{
	...
	if (gs->widget.allocation.height >= gs->height) {
        ...
        if(gs->scroll_start_x != -1 && gs->scroll_start_y != -1) {
                gdk_draw_rectangle(gs->pstarget, gs->psgc, FALSE,
...
static void
gtk_gs_unrealize (GtkWidget *widget)
{
        GtkGS  *gs;
        ...
        if(gs->pstarget) {
                gdk_window_destroy(gs->pstarget);
                gs->pstarget = NULL;
        }
        if(gs->psgc) {
                gdk_gc_destroy(gs->psgc);
                gs->psgc = NULL;
        }
...
check_filecompressed (GtkGS *gs)
{
	...
	filename_unc = g_strconcat (g_get_tmp_dir(), "/uncXXXXXX", NULL);
        fd = mkstemp (filename_unc);
        if (fd < 0) {
                g_free (filename_unc);
                return filename;
        }
        close (fd);
	filename_err = g_strconcat (g_get_tmp_dir(), "/errXXXXXX", NULL);
        fd = mkstemp (filename_err);
        if (fd < 0) {
                g_free (filename_unc);
                g_free (filename_err);
                return filename;
...
check_pdf (GtkGS *gs)
{
	...
	if ((file = fopen (filename, "r"))
		...
		gchar *fname, *filename_dsc, *filename_err, *cmd, *cmdline; 
                filename_dsc = g_strconcat (g_get_tmp_dir(), "/dscXXXXXX", NULL);
                fd = mkstemp (filename_dsc);
                if (fd < 0) {
                        g_free (filename_dsc);
                        return gs->gs_filename;
                }
		...
		cmd = g_strdup_printf (gtk_gs_defaults_get_scan_pdf_cmd(),
                                       fname, filename_dsc);
		g_free (fname);
...
gtk_gs_new (GtkAdjustment *hadj, GtkAdjustment *vadj)
{
	...
	gs = gtk_type_new(gtk_gs_get_type());
        gs->hadj = hadj;
        gs->vadj = vadj;
		...
		gs->doc = psscan(gs->gs_psfile, gs->respect_eof);
			...
			gs->pages_marked = g_new0 (gint, gs->doc->numpages);
                ...
                if (gs->override_orientation) {
                        ...
                        default:
                                gs->real_orientation = GTK_GS_ORIENTATION_NONE;
                                g_warning("Unknown orientation %d",
                                          gs->doc->orientation);
                                break;
                        }
                }
...
gtk_gs_set_page_size(GtkGS *gs, gint new_pagesize, gint pageid)
{
        ...
        if (!gs->doc) {
        ...
        else if ((gs->doc->epsf && !gs->override_size) || new_pagesize == -1) { /* epsf or bbox */
                if((gs->doc->pages) &&
                ...
                new_llx = new_lly = 0;
                if (!gs->override_size && gs->doc->size &&
        ...
        if(new_urx <= new_llx)
                new_urx = papersizes[8].width;
        if(new_ury <= new_lly)
                new_ury = papersizes[8].height;
...
gtk_gs_zoom_to_fit(GtkGS *gs, gboolean fit_width)
{
	...
	if(!fit_width) {
        ...
        if(ABS(new_zoom - gs->zoom_factor) < 0.001)
		return 0.0;
...
gint
gtk_gs_get_size_index(gchar *string, GtkGSPaperSize *size)
{
        guint idx = 0;
                        ...
                        GtkAdjustment *vadj)
{
  ...
  if (gs->hadj && (gs->hadj != hadj))
    {
      gtk_signal_disconnect_by_data (GTK_OBJECT (gs->hadj), gs);
      gtk_object_unref (GTK_OBJECT (gs->hadj));
    }
  
  if (gs->vadj && (gs->vadj != vadj))
    {
      gtk_signal_disconnect_by_data (GTK_OBJECT (gs->vadj), gs);
      gtk_object_unref (GTK_OBJECT (gs->vadj));
    }
