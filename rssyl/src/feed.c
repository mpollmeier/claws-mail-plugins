 ...
 */
static void rssyl_read_existing(RSSylFolderItem *ritem)
{
...
static gint rssyl_parse_rdf(xmlDocPtr doc, RSSylFolderItem *ritem)
{
 ...
 */
static gint rssyl_parse_rss(xmlDocPtr doc, RSSylFolderItem *ritem)
{
	xmlXPathContextPtr context;
	xmlXPathObjectPtr result;
	xmlNodePtr node, n;
 ...
 * RSSylFolderItem from it. Returns number of parsed items
 */
static gint rssyl_parse_atom(xmlDocPtr doc, RSSylFolderItem *ritem)
{
	xmlNodePtr node, n;
	gint count = 0;
	RSSylFeedItem *fitem = NULL;
	...
	for (; node; node = node->next) {
		...
		do {
			...
			if( !strcmp(n->name, "author") ) {
				...
				fitem->author = g_strdup_printf("%s%s%s%s%s",
							name ? name:"",
							name && mail ? " <":(mail?"<":""),
							mail ? mail:"",
							mail ? ">":"",
